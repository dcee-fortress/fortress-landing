...
import {
  PPE_RECEIVED_COLUMNS,
  createPpeReceivedRow,
  formatPpeReceivedDateLabel,
  getPpeReceivedRows,
  withPpeReceivedTotals,
  savePpeReceivedRows,
...
import { getPpeReceivedDailyFileHref } from "@/lib/projectRoutes"
...
export default function PpeReceivedEntryView({ projectId, projectName, dayId }) {
  const file = getDailyFile(projectId, dayId)
  const dayLabel = file?.label || dayId
  const dateLabel = formatPpeReceivedDateLabel(dayId)
  ...
  const loadRows = useCallback(() => {
    setRows(getPpeReceivedRows(projectId, dayId))
  }, [dayId, projectId])
  ...
  const persist = useCallback(
    async (nextRows) => {
      setSaveState("saving")
      try {
        await savePpeReceivedRows(projectId, dayId, nextRows)
        setSaveState("saved")
      } catch {
        setSaveState("error")
      }
  ...
  useEffect(() => {
    const flush = () => {
      window.clearTimeout(saveTimerRef.current)
      void savePpeReceivedRows(projectId, dayId, rowsRef.current)
    }
  ...
  function commitRows(updater, { immediate = false } = {}) {
    setRows((current) => {
      ...
      const next = withPpeReceivedTotals(draft)
      if (immediate) {
        void persist(next)
      } else {
  ...
  function addRow() {
    commitRows((current) => [...current, createPpeReceivedRow()], { immediate: true })
  }

  function deleteRow(rowId) {
    commitRows((current) => {
      const next = current.filter((row) => row.id !== rowId)
      return next.length > 0 ? next : [createPpeReceivedRow()]
    }, { immediate: true })
  }
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <Link
          href={getPpeReceivedDailyFileHref(projectId, dayId)}
      ...
      <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 px-4 py-3 sm:px-5">
          <div>
            <h2 className="text-base font-semibold text-zinc-900">PPE received entry table</h2>
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead>
              <tr className="bg-zinc-50 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">
                {PPE_RECEIVED_COLUMNS.map((column) => (
                  <th
                    key={column.key}
                ...
                <tr key={row.id} className="border-b border-zinc-100 align-top">
                  ...
                  <td className="px-3 py-2">
                    <textarea
                      ...
                      placeholder="Description / name of PPE received"
