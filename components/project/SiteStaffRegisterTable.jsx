...
import {
  ...
  getSiteStaffRegisterData,
  saveSiteStaffRegisterData,
} from "@/lib/siteStaffRegisterData"

const IDENTITY_COLUMNS = [
  { field: "name", label: "Name", placeholder: "Name", width: 180 },
  { field: "role", label: "Role", placeholder: "Role", width: 160 },
]
...
export default function SiteStaffRegisterTable({ projectId, monthId }) {
  const { refresh, version } = useProjectData()
  const { daysInMonth, monthName } = getMonthRegisterMeta(monthId)
  const [register, setRegister] = useState(() => getSiteStaffRegisterData(projectId, monthId))
  ...
  const loadRegister = useCallback(() => {
    ...
    const next = getSiteStaffRegisterData(projectId, monthId)
  ...
  const persist = useCallback(
    (nextRegister, { refreshAfter = false, immediate = false } = {}) => {
      ...
      const write = () => {
        saveSiteStaffRegisterData(projectId, monthId, registerRef.current)
  ...
  const addRow = () => {
    const current = registerRef.current ?? getSiteStaffRegisterData(projectId, monthId)
    const nextRegister = {
      ...current,
    ...
    registerRef.current = nextRegister
    setRegister(nextRegister)
    saveSiteStaffRegisterData(projectId, monthId, nextRegister)
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-zinc-600">
          Add a staff member, then use the day boxes beside their name.{" "}
          ...
          <tbody>
            {register.rows.length > 0 ? (
              register.rows.map((row) => (
                <tr key={row.id} className="bg-white">
                  ...
                  {dayNumbers.map((day) => (
                    <td
                      key={day}
                      className="min-w-[2.75rem] border border-zinc-200 px-1 py-1 text-center"
                    >
              ...
              <tr>
                ...
                >
                  Click &quot;Add staff row&quot; to enter a name and role. Tick boxes for{" "}
                  {monthName} appear on the same row.
                </td>
              </tr>
            )}
          </tbody>
