...
import {
  createPpeReceivedDailyFileForToday,
  deletePpeReceivedDay,
  getPpeReceivedDailyFiles,
  getPpeReceivedEntryStatus,
  migratePpeReceivedToManualOnly,
...
export default function DailyPpeReceivedListView({ projectId, projectName }) {
  ...
  const dailyFiles = useMemo(() => {
    if (!ready) return []
    void version
    return getPpeReceivedDailyFiles(projectId)
  ...
  async function handleCreateToday() {
    ...
    setCreating(true)
    try {
      const result = await createPpeReceivedDailyFileForToday(projectId)
      if (!result.ok) {
        window.alert(result.message || "Could not create today's PPE received file.")
        return
      }
      if (result.alreadyExists) {
        window.alert(result.message || "Today's PPE received file already exists.")
      }
      refresh()
    } catch (error) {
      ...
      <header className="space-y-2">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            ...
            title="Create today's PPE received file"
        ...
        {!ready ? (
          ...
          <p className="px-6 py-10 text-center text-sm text-zinc-500">
            {search.activeQuery
              ...
              : "No PPE received files yet. Press + to create today's file."}
          </p>
        )}
      </div>
