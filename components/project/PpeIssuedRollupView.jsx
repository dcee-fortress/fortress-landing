...
import {
  getPpeIssuedDashboardLinesForDayIds,
  getPpeIssuedDayIds,
  getPpeIssuedDayIdsInMonth,
  getPpeIssuedDayIdsInWeek,
} from "@/lib/ppeIssued"
import { getMonthlyFiles, getWeeklyFiles } from "@/lib/projectFiles"
import { getPpeIssuedHref } from "@/lib/projectRoutes"

export default function PpeIssuedRollupView({
  projectId,
  projectName,
  title,
  description,
  ...
  const periods = useMemo(() => {
    ...
    if (mode === "project-to-date") {
      const dayIds = getPpeIssuedDayIds(projectId)
      return [
        {
          id: "project-to-date",
          label: "Project to date",
          dayIds,
          lines: getPpeIssuedDashboardLinesForDayIds(projectId, dayIds),
        },
      ]
    }
    ...
    return files
      .map((file) => {
        const dayIds =
          mode === "weekly"
            ? getPpeIssuedDayIdsInWeek(projectId, file.id)
            : getPpeIssuedDayIdsInMonth(projectId, file.id)
        return {
          id: file.id,
          label: file.label,
          dayIds,
  ...
  function exportToPdf() {
    void import("@/lib/safetyReportPdf").then(({ exportPpeIssuedDashboardPdf }) => {
      const lines = visiblePeriods.flatMap((period) => period.lines || [])
      exportPpeIssuedDashboardPdf({
        ...
        periodLabel:
          mode === "project-to-date"
            ? "Project to date"
            : visiblePeriods.map((period) => period.label).join(", ") || title,
        lines,
      })
    })
  }
      ...
      {!hasHydrated ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
          ...
          <PpeIssuedDashboardTable
            lines={[]}
            emptyMessage="No PPE issued in this period yet. Add entries on daily PPE issued."
          />
        </section>
      ) : (
            ...
            >
              <div className="border-b border-zinc-200 px-4 py-3 sm:px-5">
                ...
                <p className="mt-0.5 text-sm text-zinc-500">
                  Rolled up from daily PPE issued entry · matching descriptions combined
                </p>
              </div>
              <PpeIssuedDashboardTable lines={period.lines} />
            </section>
          ))}
        </div>
      )}
