...
import {
  deleteGoodsReceivedDay,
  getGoodsReceivedDeletedDayIds,
  getGoodsReceivedEntryStatus,
} from "@/lib/goodsReceived"
import { getDailyFiles } from "@/lib/projectFiles"
import { getGoodsReceivedDailyFileHref, getGoodsReceivedHref } from "@/lib/projectRoutes"
...
const DailyGoodsReceivedRow = memo(function DailyGoodsReceivedRow({
  file,
  projectId,
  status,
  selected,
  ...
  return (
    ...
    >
      <Link
        href={getGoodsReceivedDailyFileHref(projectId, file.id)}
        prefetch={false}
      ...
      <button
        ...
        title="Delete daily goods received file"
        disabled={deleting}
        onClick={(event) => {
...
export default function DailyGoodsReceivedListView({ projectId, projectName }) {
  ...
  const deletedDayIds = useMemo(() => {
    ...
    void version
    return new Set(getGoodsReceivedDeletedDayIds(projectId))
  ...
  async function performDeleteDailyFile(file) {
    const confirmed = window.confirm(
      `Delete "${file.label}" from daily goods received?\n\nThis removes the day's entry from weekly, monthly, and project-to-date rollups.`
    )
    if (!confirmed) return

    setDeletingDayId(file.id)
    try {
      const result = await deleteGoodsReceivedDay(projectId, file.id)
      if (!result.ok) {
  ...
  async function performDeleteSelected() {
    ...
    try {
      for (const dayId of ids) {
        const result = await deleteGoodsReceivedDay(projectId, dayId)
        if (!result.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <ReportFileSearchBar
          {...search}
          projectId={projectId}
          getFileHref={getGoodsReceivedDailyFileHref}
          placeholder="Search daily goods received files"
        />
        ...
        {!hasHydrated ? (
          ...
          <ul className="divide-y divide-zinc-200">
            {visibleFiles.map((file) => {
              const status = getGoodsReceivedEntryStatus(projectId, file)
              return (
                <DailyGoodsReceivedRow
                  key={file.id}
                  file={file}
                  projectId={projectId}
                  status={status}
