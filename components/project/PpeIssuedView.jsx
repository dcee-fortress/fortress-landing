...
import {
  PPE_ISSUED_PERIODS,
  getPpeIssuedPeriodHref,
  getPersonalProtectiveEquipmentHref,
} from "@/lib/projectRoutes"

export default function PpeIssuedView({ projectId, projectName }) {
  return (
    <div className="space-y-6">
      ...
      <div className="app-choice-grid">
        {PPE_ISSUED_PERIODS.map((item) => (
          <ChoiceCard
            key={item.period}
            href={getPpeIssuedPeriodHref(projectId, item.period)}
            icon={item.icon}
            title={item.label}
