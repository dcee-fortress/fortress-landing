...
import {
  deleteGoodsAcquiredDay,
  getGoodsAcquiredDeletedDayIds,
  getGoodsAcquiredEntryStatus,
} from "@/lib/goodsAcquired"
import { getDailyFiles } from "@/lib/projectFiles"
import { getGoodsAcquiredDailyFileHref, getGoodsAcquiredHref } from "@/lib/projectRoutes"
...
const DailyGoodsAcquiredRow = memo(function DailyGoodsAcquiredRow({
  file,
  projectId,
  status,
  selected,
  ...
  return (
    ...
    >
      <Link
        href={getGoodsAcquiredDailyFileHref(projectId, file.id)}
        prefetch={false}
...
export default function DailyGoodsAcquiredListView({ projectId, projectName }) {
  ...
  const deletedDayIds = useMemo(() => {
    ...
    void version
    return new Set(getGoodsAcquiredDeletedDayIds(projectId))
  ...
  async function performDeleteDailyFile(file) {
    ...
    try {
      const result = await deleteGoodsAcquiredDay(projectId, file.id)
      if (!result.ok) {
  ...
  async function performDeleteSelected() {
    ...
    try {
      for (const dayId of ids) {
        const result = await deleteGoodsAcquiredDay(projectId, dayId)
        if (!result.ok) {
        ...
        {!hasHydrated ? (
          ...
          <ul className="divide-y divide-zinc-200">
            {visibleFiles.map((file) => {
              const status = getGoodsAcquiredEntryStatus(projectId, file)
              return (
                <DailyGoodsAcquiredRow
                  key={file.id}
                  file={file}
                  projectId={projectId}
                  status={status}
