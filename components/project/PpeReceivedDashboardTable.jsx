...
export const PPE_RECEIVED_DASHBOARD_COLUMNS = [
  { key: "description", label: "PPE received", align: "left" },
  { key: "quantity", label: "Quantities", align: "right" },
  { key: "totalCost", label: "Total cost", align: "right" },
]

export default function PpeReceivedDashboardTable({ lines, emptyMessage }) {
  ...
  if (rows.length === 0) {
    return (
      <p className="px-4 py-8 text-center text-sm text-zinc-500 sm:px-5">
        {emptyMessage || "No PPE received entries yet."}
      </p>
    )
  }
      ...
      <table className="min-w-full border-collapse text-sm">
        <thead>
          <tr className="bg-zinc-50 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">
            {PPE_RECEIVED_DASHBOARD_COLUMNS.map((column) => (
              <th
                key={column.key}
