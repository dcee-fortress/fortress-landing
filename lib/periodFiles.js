...
import { parseProjectDayId } from "@/lib/dailyFiles"
...
export function getProjectStartDate(projectId) {
  ...
  if (project?.startDate) {
    return parseProjectDayId(project.startDate)
  }

  if (projectId === "1") {
    return parseProjectDayId(CHADCOM_START_DATE)
  }
...
export function resolveWeeklyFiles(projectId, baseFiles) {
  ...
  return mergeFileLists(baseFiles, extras.filter(Boolean), isWeekPeriodComplete).filter(
    (file) => {
      const project = getCustomProjectById(projectId)
      return !project?.startDate || file.id >= project.startDate
    }
  )
}
...
export function resolveMonthlyFiles(projectId, baseFiles) {
  ...
  return mergeFileLists(baseFiles, extras.filter(Boolean), isMonthPeriodComplete).filter(
    (file) => {
      const project = getCustomProjectById(projectId)
      if (!project?.startDate) return true
      return file.id >= project.startDate.slice(0, 7)
    }
  )
}
...
export function shouldShowWeeklyFile(_projectId, _file) {
  return true
}

export function shouldShowMonthlyFile(_projectId, _file) {
  return true
}
