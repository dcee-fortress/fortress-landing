...
import { addDays, dayIdFromDate, parseDayId } from "@/lib/dailyFiles"
...
function writeStore(store) {
  ...
  const currentRaw = window.localStorage.getItem(PROJECT_DATA_STORAGE_KEY)
  if (storeCacheRaw !== null && currentRaw !== storeCacheRaw) {
    storeCache = null
    storeCacheRaw = null
    return
  }
...
export function ensureHourlyDashboardsForProject(projectId) {
  ...
  const created = ensureHourlyDashboardsForDateRange(projectId, project.startDate, throughDayId)
  if (created) {
