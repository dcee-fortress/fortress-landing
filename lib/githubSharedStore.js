import { mergeSharedStorageMaps } from "@/lib/sharedStorageMerge"
...
function snapshotUrls() {
  ...
  const ref = githubRef()
  return [`https://raw.githubusercontent.com/${repo}/${ref}/data/grove-shared-storage.json`]
}
  ...
  try {
    ...
    for (const url of snapshotUrls()) {
      try {
        ...
        merged = merged ? mergeSharedStorageMaps(merged, parsed) : parsed
      } catch {
