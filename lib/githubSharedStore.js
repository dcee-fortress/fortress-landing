...
async function readGithubApiSnapshot() {
  ...
  return parseSnapshot(parseJsonText(decoded))
}
...
export async function readGithubSharedStore() {
  ...
  try {
    ...
    const parsed = parseSnapshot(parseJsonText(await response.text()))
    if (parsed) {
      cache.data = parsed
      cache.at = Date.now()
    }
...
export async function writeGithubSharedStore(storage) {
  ...
  try {
    ...
    const currentJson = current.ok ? await readResponseJson(current, {}) : {}
