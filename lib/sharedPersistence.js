  ...
  const pull = async (options = {}) => {
    ...
    try {
      ...
      const shouldSeedLive =
        scoreSharedStorage(shared) <= 0 &&
        scoreSharedStorage(localSnapshot) > scoreSharedStorage(shared) &&
        localHasRestorableProject
  ...
  const synchronize = async (reason = "poll") => {
    ...
    try {
      do {
        ...
        if (!initialized) {
          const pulled = await pull({ ensureCalendar: false, remoteWins: true })
          initialized = true
          resolveReady()
          if (pulled) {
