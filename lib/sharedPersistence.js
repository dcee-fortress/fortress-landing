...
function createSharedPersistenceSession() {
    ...
    try {
      do {
        ...
        if (!initialized) {
          ...
          const remoteHasData = countRegistryProjects(remoteSnapshot) > 0
          const localHasData = countRegistryProjects(localSnapshot) > 0
          const canUpload = shouldPublishSharedData()
          if (canUpload && localHasData && countRegistryProjects(localSnapshot) > countRegistryProjects(remoteSnapshot)) {
            try {
        ...
        try {
          ...
          const remoteHasData = countRegistryProjects(remoteSnapshot) > 0
          if (shouldPublishSharedData() && countRegistryProjects(localSnapshot) > countRegistryProjects(remoteSnapshot)) {
            try {
  ...
  window.localStorage.setItem = (key, value) => {
    originalSetItem(key, value)
    if (!syncKeySet.has(key) || applyingRemote || isSystemStorageWrite()) return
    lastUserEditAt.set(key, Date.now())
    markSharedStorageLocalWrite(key)
...
export async function publishSharedKeys(options = {}) {
  ...
  for (const key of orderedKeys) {
    ...
    try {
      ...
      // Publish remaining keys so a large snapshot cannot block the project list.
    }
  }
}
