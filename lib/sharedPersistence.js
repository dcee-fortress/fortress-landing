...
import { mergeSharedStorageValue, PROJECTS_REGISTRY_KEY, purgeDeletedProjectsFromStorage, stripDemoSharedValue } from "@/lib/sharedStorageMerge"
import {
  ...
  const pull = async (options = {}) => {
    ...
    try {
      ...
      for (const key of syncKeys) {
        ...
        const mergedValue = mergeSharedStorageValue(key, localValue, liveValue)
        if (mergedValue !== localValue) {
          setLocalValue(key, mergedValue)
          changed = true
        }
        knownValues.set(key, mergedValue)
      }
      ...
      try {
        ...
        const kept =
          key === PROJECTS_REGISTRY_KEY
            ? mergeSharedStorageValue(key, value, published)
            : published
        if (kept !== value) {
          setLocalValue(key, kept)
          mergedFromServer = true
        }
        knownValues.set(key, kept)
