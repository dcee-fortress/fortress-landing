...
function withTodayFlags(file, todayId) {
  ...
  return {
    ...
    completedAt: isToday ? "Awaiting entry" : file.completedAt === "Awaiting entry" ? file.date : file.completedAt,
  }
}
...
function ensureCustomProjectDailyFilesThroughToday(projectId) {
  ...
  if (!registry.files[projectId]) {
    registry.files[projectId] = emptyProjectFiles()
  }
  ...
  while (cursor <= through) {
    ...
    if (!existingIds.has(id)) {
      newFiles.push(createBlankDailyFile(cursor, { isToday: id === todayId }))
    }
    cursor = addDays(cursor, 1)
  }

  const nextDaily = [...newFiles, ...daily]
  ...
  const flagsChanged = daily.some((file) => {
    ...
    return (
      file.isToday !== next.isToday ||
      file.awaitingEntry !== next.awaitingEntry ||
      file.completedAt !== next.completedAt
    )
  })
  const changed = newFiles.length > 0 || flagsChanged
  ...
  return changed || reportsChanged || registerChanged
}
