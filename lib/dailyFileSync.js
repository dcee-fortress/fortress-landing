...
import { hasMaterialScheduleDataForDay } from "@/lib/materialSchedule"

export function dayHasSavedHourlyData(projectId, dayId, savedDayIds = null) {
  if (hasMaterialScheduleDataForDay(projectId, dayId)) return true

  // Days with saved hourly slots still count as entered even before schedule rows exist.
  if (savedDayIds) {
...
/** Stable status for SSR/first paint — does not read client cache. */
...
export function resolveDailyFiles(projectId, baseFiles) {
  ...
  const deletedDailyIds = new Set(
    readProjectsRegistry().files?.[projectId]?.deletedDailyIds ?? []
  )
  ...
  while (cursor <= today) {
    ...
    if (!deletedDailyIds.has(id)) {
      extras.push(createBlankDailyFile(cursor, { isToday: isTodayDayId(id) }))
    }
    cursor = addDays(cursor, 1)
  }

  return mergeDailyFileLists(baseFiles, extras).filter(
    (file) => (!startId || file.id >= startId) && !deletedDailyIds.has(file.id)
  )
}
...
export function getResolvedDailyFiles(projectId, baseFiles) {
  ...
  if (typeof window !== "undefined") {
    ...
    const files = projectFiles?.daily ?? baseFiles
    const deletedDailyIds = new Set(projectFiles?.deletedDailyIds ?? [])
    const startDate = project?.startDate
    const withoutDeleted = files.filter((file) => !deletedDailyIds.has(file.id))
    const bounded = startDate
      ? withoutDeleted.filter((file) => file.id >= startDate)
      : withoutDeleted

    if (isProjectEnded(projectId)) {
      const throughDayId = getProjectEffectiveThroughDate(projectId)
      return bounded
        .filter((file) => file.id <= throughDayId)
        .sort((left, right) => right.id.localeCompare(left.id))
    }

    // Always fill through today and sort newest-first so today's file appears
    // at the top even if sync left the registry list unsorted or incomplete.
    return resolveDailyFiles(projectId, bounded).filter((file) => !deletedDailyIds.has(file.id))
  }
