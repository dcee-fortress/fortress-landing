...
export function keepRicherRegistryValue(localValue, incomingValue) {
  ...
  const incomingIds = new Set((right.projects ?? []).map((project) => project?.id).filter(Boolean))
  const deletedIds = mergeDeletedProjectIdLists(left.deletedIds ?? [], right.deletedIds ?? []).filter(
