...
function pickRicherFileList(left = [], right = []) {
  const leftList = Array.isArray(left) ? left : []
  const rightList = Array.isArray(right) ? right : []
  if (rightList.length === 0) return leftList
  if (leftList.length === 0) return rightList
...
function mergeProjectFileBuckets(leftBucket, rightBucket) {
  ...
  return {
    daily: pickRicherFileList(left.daily, right.daily),
    weekly: pickRicherFileList(left.weekly, right.weekly),
    monthly: pickRicherFileList(left.monthly, right.monthly),
    deletedDailyIds: [
      ...new Set([...(left.deletedDailyIds ?? []), ...(right.deletedDailyIds ?? [])].filter(Boolean)),
    ],
    progressReports: pickRicherFileList(left.progressReports, right.progressReports),
    weeklyProgressReports: pickRicherFileList(left.weeklyProgressReports, right.weeklyProgressReports),
    dailyProgressReports: pickRicherFileList(left.dailyProgressReports, right.dailyProgressReports),
  }
}
