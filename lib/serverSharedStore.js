...
async function loadStore() {
  ...
  const hasPersistedRegistry = typeof persisted[PROJECTS_REGISTRY_KEY] === "string"
  const merged = hasPersistedRegistry ? persisted : mergeSharedStorageMaps(bundled, persisted)
  const cleaned = applyDeletedProjectIds(
    stripDemoSharedStorage(merged),
    collectDeletedProjectIds(
      hasPersistedRegistry ? persisted : bundled,
      github,
      disk,
      memory,
      postgres,
...
export async function writeSharedValue(key, incomingValue, options = {}) {
  ...
  return enqueueWrite(async () => {
    ...
    const storage = await loadStore()
    storage[key] = replace ? incomingValue : mergeSharedStorageValue(key, storage[key], incomingValue)
    const extraDeleted =
    ...
    const next = replace ? storage : applyDeletedProjectIds(storage, extraDeleted)
    await persistPurgedStore(next)
    return next[key]
  })
}
