...
async function persistPurgedStore(purged) {
  ...
  if (postgresAvailable()) {
    ...
    if (current && typeof purged[PROJECTS_REGISTRY_KEY] === "string") {
      purged[PROJECTS_REGISTRY_KEY] = keepRicherRegistryValue(
        current[PROJECTS_REGISTRY_KEY],
        purged[PROJECTS_REGISTRY_KEY]
      )
    }
  }
...
export async function replaceSharedStorage(incomingStorage = {}) {
  return enqueueWrite(async () => {
    ...
    if (countRegistryProjects(nextStorage) < countRegistryProjects(existing)) {
    ...
    await persistStore(nextStorage)
    ...
    for (const key of SHARED_STORAGE_KEYS) {
      try {
        await upsertPostgresValue(key, nextStorage[key])
      } catch {
  ...
  return enqueueWrite(async () => {
    ...
    for (const [key, incomingValue] of Object.entries(updates)) {
      ...
      if (key === PROJECTS_REGISTRY_KEY) {
        ...
        for (const projectId of previouslyDeleted) {
          if (!liveIds.has(projectId)) delete files[projectId]
        }
        storage[key] = JSON.stringify({
          ...registry,
          projects,
          files,
          deletedIds: mergeDeletedProjectIdLists(registry.deletedIds ?? [], [...previouslyDeleted]).filter(
            (id) => !liveIds.has(id)
          ),
        })
      }
