...
async function readPostgresStore() {
  ...
  const rows = await prisma.groveSharedStorage.findMany({
    select: { storageKey: true, storageValue: true },
  })

  return Object.fromEntries(rows.map(({ storageKey, storageValue }) => [storageKey, storageValue]))
}
...
async function upsertPostgresValue(key, value) {
  ...
  await ensureStorageTable()
  await prisma.groveSharedStorage.upsert({
    where: { storageKey: key },
    create: { storageKey: key, storageValue: value },
    update: { storageValue: value },
  })
  return true
}
...
async function deletePostgresValue(key) {
  ...
  await ensureStorageTable()
  await prisma.groveSharedStorage.deleteMany({ where: { storageKey: key } })
  return true
}
...
async function withPostgresLock(work) {
  if (!prismaAvailable()) return work()

  await ensureStorageTable()
  await prisma.$executeRawUnsafe(`SELECT pg_advisory_lock(${STORAGE_LOCK_KEY})`)
  try {
    return await work()
  } finally {
    await prisma.$executeRawUnsafe(`SELECT pg_advisory_unlock(${STORAGE_LOCK_KEY})`)
  }
}
...
async function persistChangedKeys(storage, keys, options = {}) {
  if (process.env.VERCEL && !prismaAvailable()) {
...
async function ensureDurableCopy(storage) {
  if (!prismaAvailable() || scoreSharedStorage(storage) <= 0) return
...
export async function writeSharedValue(key, incomingValue, options = {}) {
  ...
  return enqueueWrite(async () => {
    ...
    const extraDeleted = collectDeletedProjectIds(storage).filter((id) => !liveIds.has(id))
    const next = applyDeletedProjectIds(storage, extraDeleted)
    await persistChangedKeys(next, [key, PROJECTS_REGISTRY_KEY, DELETED_PROJECT_IDS_KEY])
    return next[key]
  })
}
