...
export async function replaceSharedStorage(incomingStorage = {}) {
  return enqueueWrite(async () => {
    ...
    const nextStorage = applyDeletedProjectIds(storage, RETIRED_PROJECT_IDS)
  ...
  return enqueueWrite(async () => {
    ...
    if (key === PROJECTS_REGISTRY_KEY) {
      ...
      const projects = (registry.projects ?? []).filter(
        (project) => project?.id && !previouslyDeleted.has(project.id)
      )
      const files = { ...(registry.files ?? {}) }
      for (const projectId of previouslyDeleted) delete files[projectId]
      storage[key] = JSON.stringify({
        ...registry,
        projects,
        files,
        deletedIds: mergeDeletedProjectIdLists(registry.deletedIds ?? [], [...previouslyDeleted]),
      })
    }
