...
async function ensureDurableCopy(storage) {
  if (!postgresAvailable() || !storeHasRegistry(storage)) return

  try {
    const existing = await readPostgresStore()
    if (storeHasRegistry(existing)) return
...
async function loadStore() {
  ...
  // Durable shared stores win over this serverless instance's memory.
  // Using memory first made each device/instance show a different copy.
  const authoritative = storeHasRegistry(postgres)
    ? postgres
    : storeHasRegistry(github)
      ? github
      : storeHasRegistry(disk)
        ? disk
        : storeHasRegistry(memory)
          ? memory
          : bundled

  const tombstones = collectDeletedProjectIds(bundled, github, disk, memory, postgres, authoritative)
