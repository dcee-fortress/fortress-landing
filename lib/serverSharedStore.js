...
async function loadStore() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (live && typeof live === "object" && !live.error) {
          ...
          const cleaned = applyDeletedProjectIds(
            mergeSharedStorageMaps(bundled, live),
            collectDeletedProjectIds(bundled, live)
          )
  ...
  const persisted = mergeSharedStorageMaps(
    mergeSharedStorageMaps(mergeSharedStorageMaps(github ?? {}, disk), memory),
    postgres ?? {}
  )
  const merged = mergeSharedStorageMaps(bundled, persisted)
  const cleaned = applyDeletedProjectIds(
    stripDemoSharedStorage(merged),
    collectDeletedProjectIds(bundled, github, disk, memory, postgres, persisted, merged)
  )
  setMemoryStore(cleaned)
  return cleaned
}
...
export async function readSharedStorage() {
  ...
  if (nextValue !== currentValue) {
    ...
    await persistPurgedStore(purged)
    return purged
  }
...
export async function writeSharedValue(key, incomingValue) {
  return enqueueWrite(async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.value === "string") {
            const kept = mergeSharedStorageValue(key, incomingValue, data.value)
            const storage = { ...memoryStore(), [key]: kept }
            const cleaned =
              key === PROJECTS_REGISTRY_KEY
                ? applyDeletedProjectIds(
                    storage,
                    collectDeletedProjectIds({ [PROJECTS_REGISTRY_KEY]: incomingValue }, storage)
                  )
                : storage
            setMemoryStore(cleaned)
            await writeDiskStore(cleaned)
            return cleaned[key] ?? kept
          }
        }
      } catch {
    ...
    const extraDeleted =
      key === PROJECTS_REGISTRY_KEY
        ? collectDeletedProjectIds({ [PROJECTS_REGISTRY_KEY]: incomingValue }, storage)
        : collectDeletedProjectIds(storage)
    const purged = applyDeletedProjectIds(storage, extraDeleted)
    await persistPurgedStore(purged)
