...
async function ensureDurableCopy(storage) {
  ...
  try {
    ...
    if (scoreSharedStorage(existing) >= scoreSharedStorage(storage)) return
...
async function loadStore() {
  if (shouldSyncLiveData()) {
    try {
      ...
      // Fall through to the last live snapshot kept in memory.
    }

    const previous = memoryStore()
    if (storeHasRegistry(previous)) {
      return previous
    }
  }
  ...
  const authoritative = pickRichestSharedStorage(postgres, github, disk, memory, bundled) ?? bundled
