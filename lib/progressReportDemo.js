import { formatOperatingHours, resolveEquipmentOperatingHours } from "@/lib/equipmentHoursData"
...
const HOURS_CELL =
  'style="border:1px solid #d4d4d8; padding:8px 10px; text-align:right; white-space:nowrap; min-width:5.5rem; font-weight:600;"'
const HOURS_HEAD =
  'style="border:1px solid #d4d4d8; padding:8px 10px; text-align:right; background:#f4f4f5; white-space:nowrap; min-width:5.5rem;"'
const HOURS_FOOT =
  'style="border:1px solid #d4d4d8; padding:8px 10px; text-align:right; font-weight:700; white-space:nowrap; min-width:5.5rem;"'
...
function equipmentRowHours(item) {
  if (item?.hoursOperating != null && item.hoursOperating !== "") {
    const numeric = Number(item.hoursOperating)
    if (Number.isFinite(numeric)) return numeric
  }

  return resolveEquipmentOperatingHours({
    startHours: item?.startHours,
    finishHours: item?.finishHours,
    hoursOperating: item?.hoursOperatingInput || item?.hoursOperating,
    hoursOperatingEdited: item?.hoursOperatingEdited,
  })
}
...
export function buildDailyValuationProgressContent(summary, label = VALUATION_NOTES_HEADING) {
  ...
  return `
    ...
    <table style="width:100%; min-width:720px; border-collapse:collapse; margin:12px 0;">
      <thead>
        <tr>
...
export function buildWeeklyEquipmentProgressContent(equipmentReport, label = EQUIPMENT_NOTES_HEADING) {
  ...
  const rowsHtml = equipmentReport.equipment
    .map((item) => {
      ...
      const hoursOperating = escapeHtml(formatHoursCell(equipmentRowHours(item)))
      ...
      return `
        <tr>
          ...
          <td ${HOURS_CELL}>${hoursOperating}</td>
        </tr>`
    })
    .join("")
    ...
    <table style="width:100%; min-width:720px; border-collapse:collapse; margin:12px 0;">
      <thead>
        <tr>
          ...
          <th ${HOURS_HEAD}>Hours operating</th>
        </tr>
      </thead>
      <tbody>${rowsHtml}</tbody>
      <tfoot>
        <tr>
          ...
          <td ${HOURS_FOOT}>${escapeHtml(`Total ${formatHoursCell(equipmentReport.totalHours)}`)}</td>
        </tr>
      </tfoot>
    </table>
...
function hasProgressHeading(html, pattern) {
  return pattern.test(String(html || ""))
}

const NOTES_HEADING_PATTERN =
  /<(?:p|h[1-3])[^>]*>\s*<strong>\s*(NOTES|Daily Valuation Progress Report)\s*<\/strong>/i
const OPERATOR_HEADING_PATTERN =
  /<(?:p|h[1-3])[^>]*>\s*<strong>\s*OPERATOR REGISTER\s*<\/strong>/i
const EQUIPMENT_HEADING_PATTERN =
  /<(?:p|h[1-3])[^>]*>\s*<strong>\s*EQUIPMENT IN USE\s*<\/strong>/i

function stripEquipmentAutoBlock(html) {
  return String(html || "")
    .replace(EQUIPMENT_HEADING_PATTERN, "")
    .replace(/<table[\s\S]*?<\/table>/gi, (table) =>
      /Hours operating/i.test(table) && /Supplier/i.test(table) ? "" : table
    )
    .replace(/<p[^>]*>These figures are copied from the equipment in use dashboard[\s\S]*?<\/p>/gi, "")
}
...
export function mergeValuationNotesContent(
  ...
  if (options.userEdited) {
    if (isEmptyHtml(saved)) {
      return snapshot || saved
    }

    let next = saved
    if (valuationHtml && !hasProgressHeading(next, NOTES_HEADING_PATTERN)) {
      next = `${valuationHtml}${next}`
    }
    if (operatorHtml && !hasProgressHeading(next, OPERATOR_HEADING_PATTERN)) {
      next = `${next}${operatorHtml}`
    }
    if (equipmentHtml) {
      next = `${stripEquipmentAutoBlock(next).trim()}${equipmentHtml}`
    }
    return next
  }

  const commentary = extractValuationNotesCommentary(saved)
  return [valuationHtml, operatorHtml, equipmentHtml, commentary].filter(Boolean).join("") || saved
}

/**
 * Fill NOTES, operator register, and equipment tables from the current
 * project's live dashboards. After the user edits the document, missing
 * tables are still inserted once; existing wording is kept.
 */
export function resolveActualProgressUpdateContent(
  ...
  const seeded = mergeValuationNotesContent(
    ...
    equipmentReport,
    operatorRegister,
    { userEdited: Boolean(report?.progressUpdate?.userEdited) }
  )
  ...
  return savedContent || report?.progressSummary || ""
}
