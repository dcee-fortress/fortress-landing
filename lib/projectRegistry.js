...
export function isDeletedProjectId(projectId) {
  ...
  try {
    const extra = JSON.parse(window.localStorage.getItem(DELETED_PROJECT_IDS_KEY) ?? "[]")
...
export function deleteCustomProject(projectId) {
  ...
  const ids = (() => {
    try {
      const previous = JSON.parse(window.localStorage.getItem(DELETED_PROJECT_IDS_KEY) ?? "[]")
