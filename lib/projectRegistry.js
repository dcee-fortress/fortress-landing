...
export const DELETED_SAVED_PROJECTS_STORAGE_KEY = "grove-deleted-saved-projects"
...
const deletedProjectsStore = createLocalStorageCache(DELETED_SAVED_PROJECTS_STORAGE_KEY, [])
...
export function deleteCustomProject(projectId) {
  ...
  const files = registry.files[projectId] ?? null

  if (project) {
    const deletedProjects = deletedProjectsStore.read()
    deletedProjects.push({
      deletedAt: new Date().toISOString(),
      project,
      files,
    })
    deletedProjectsStore.write(deletedProjects)
  }
...
export function getDeletedSavedProjects() {
  if (typeof window === "undefined") return []

  const deletedProjects = deletedProjectsStore.read()
  return Array.isArray(deletedProjects) ? [...deletedProjects].reverse() : []
}

export function clearDeletedSavedProjects() {
  if (typeof window === "undefined") return
  window.localStorage.removeItem(DELETED_SAVED_PROJECTS_STORAGE_KEY)
  deletedProjectsStore.invalidate()
}
