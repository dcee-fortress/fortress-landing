...
export function deleteCustomProject(projectId) {
  ...
  try {
    ...
    const ids = [...new Set([...(Array.isArray(previous) ? previous : []), projectId])]
    window.localStorage.setItem(DELETED_PROJECT_IDS_KEY, JSON.stringify(ids))
  } catch {
    window.localStorage.setItem(DELETED_PROJECT_IDS_KEY, JSON.stringify([projectId]))
  }
  clearProjectSyncSessionKeys(projectId)
  return true
}
