...
import { DELETED_PROJECT_IDS_KEY } from "@/lib/sharedStorageMerge"
...
export const CHADCOM_START_DATE = "2026-08-28"
...
const EMPTY_REGISTRY = { projects: [], files: {}, deletedIds: [] }
...
function writeRegistry(registry) {
  ...
  try {
    return registryStore.write(registry)
  } catch (error) {
...
export function generateBlankMonthlyFiles(projectStart, throughDay) {
  const files = []
...
function rebuildProjectPeriodFilesInRegistry(registry, projectId, startDate, throughDate) {
  ...
  const start = parseProjectDayId(startDate)
  const through = parseProjectDayId(throughDate)

  if (start > through) {
    return
  }

  const deletedDailyIds = new Set(projectFiles.deletedDailyIds ?? [])
  ...
  const daily = mergeFilesById(projectFiles.daily, generatedDaily).filter(
    (file) =>
      file.id >= startDate &&
      file.id <= dayIdFromDate(through) &&
      !deletedDailyIds.has(file.id)
  )
  ...
  const generatedDailyProgress =
    start <= through ? generateDailyProgressReports(start, through) : []
  ...
  registry.files[projectId] = {
    ...
    weeklyProgressReports: mergeFilesById(
      projectFiles.weeklyProgressReports ?? projectFiles.progressReports,
      generatedProgress
    ),
    dailyProgressReports: mergeFilesById(projectFiles.dailyProgressReports, generatedDailyProgress),
  }
}
...
function buildInitialFiles(startDate, throughDate = null) {
  const start = parseProjectDayId(startDate)
  const through = parseProjectDayId(throughDate ?? getTodayDayId())
  ...
  return {
    ...
    weeklyProgressReports: start <= through ? generateProgressReports(start, through) : [],
    dailyProgressReports: start <= through ? generateDailyProgressReports(start, through) : [],
  }
}
...
export function isEndedProject(project) {
  if (!project) return false
  const normalized = normalizeProjectRecord(project)
  return normalized.status === PROJECT_STATUS.ENDED || normalized.active === false
}
...
export function isDeletedProjectId(projectId) {
  ...
  if ((registry.deletedIds ?? []).includes(projectId)) return true

  if (typeof window === "undefined") return false

  try {
    const extra = JSON.parse(getGroveItem(DELETED_PROJECT_IDS_KEY) ?? "[]")
    return Array.isArray(extra) && extra.includes(projectId)
  } catch {
    return false
  }
}
...
export function getCustomProjects() {
  return readRegistry()
    .projects.map(normalizeProjectRecord)
    .filter((project) => project?.id && !isDeletedProjectId(project.id))
}
...
export function createCustomProject(name, options = {}) {
  ...
  registry.deletedIds = (registry.deletedIds ?? []).filter((entry) => entry !== id)
  registry.projects = [...registry.projects, project]
  registry.files[id] = buildInitialFiles(startDate)
  if (typeof window !== "undefined") {
    try {
      const extra = JSON.parse(getGroveItem(DELETED_PROJECT_IDS_KEY) ?? "[]")
      if (Array.isArray(extra) && extra.includes(id)) {
        setGroveItem(
          DELETED_PROJECT_IDS_KEY,
          JSON.stringify(extra.filter((entry) => entry !== id))
        )
      }
    } catch {
      // Registry write still creates the project.
    }
  }
  return Promise.resolve(writeRegistry(registry)).then(() => project)
}
...
export function endCustomProject(projectId, endDate = null) {
  ...
  const write = writeRegistry(registry)
  clearProjectSyncSessionKeys(projectId)

  return Promise.resolve(write).then(() => endedProject)
}
...
export function migrateRegistryProjects() {
  ...
  for (const project of registry.projects) {
    const createdDayId = project.createdAt ? dayIdFromTimestamp(project.createdAt) : null
    if (
      project.id?.startsWith("p-") &&
      project.startDate === CHADCOM_START_DATE &&
      createdDayId &&
      createdDayId > CHADCOM_START_DATE
    ) {
      project.startDate = createdDayId
      changed = true
    }
    ...
    const beforeFiles = JSON.stringify(registry.files[project.id] ?? {})
    rebuildProjectPeriodFilesInRegistry(
      registry,
      project.id,
      project.startDate,
    ...
    const afterFiles = JSON.stringify(registry.files[project.id] ?? {})
...
export async function deleteCustomProject(projectId, options = {}) {
  if (projectId === SEEDED_PROJECT_ID || !projectId.startsWith("p-")) {
    return false
  }

  const registry = readRegistry()
  registry.deletedIds = [...new Set([...(registry.deletedIds ?? []), projectId])]
  registry.projects = registry.projects.filter((entry) => entry.id !== projectId)
  delete registry.files[projectId]

  // Optimistic local update so Settings can drop the row immediately.
  const registryJson = JSON.stringify(registry)
  setGroveItem(PROJECTS_REGISTRY_KEY, registryJson)
  registryStore.invalidate()

  const ids = (() => {
    try {
      const previous = JSON.parse(getGroveItem(DELETED_PROJECT_IDS_KEY) ?? "[]")
      return [...new Set([...(Array.isArray(previous) ? previous : []), projectId])]
    } catch {
      return [projectId]
    }
  })()
  setGroveItem(DELETED_PROJECT_IDS_KEY, JSON.stringify(ids))
  clearProjectSyncSessionKeys(projectId)

  const persist = async () => {
    const { writeGroveJson } = await import("@/lib/saveToPostgres")
    await writeGroveJson(PROJECTS_REGISTRY_KEY, registry, { replace: true })
    await writeGroveJson(DELETED_PROJECT_IDS_KEY, ids, { replace: true })
  }

  if (options.waitForPersist === true) {
    await persist()
  } else {
    void persist().catch(() => {
      // Background sync will retry; UI already reflects the deletion.
    })
  }
...
export function invalidateProjectsRegistryCache() {
  registryStore.invalidate()
}

export function clearProjectsRegistry() {
  if (typeof window === "undefined") return
  removeGroveItem(PROJECTS_REGISTRY_KEY)
  registryStore.invalidate()
}
