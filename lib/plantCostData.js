...
function readStore() {
  ...
  try {
    const raw =
      getGroveItem(PLANT_COST_STORAGE_KEY) ??
      getGroveItem(LEGACY_FUEL_COST_STORAGE_KEY)
...
function writeStore(store) {
  ...
  return import("@/lib/saveToPostgres").then(({ writeGroveJson }) =>
    writeGroveJson(PLANT_COST_STORAGE_KEY, store, { replace: true })
  )
}
...
export function savePlantCostSlotRows(projectId, dayId, slotId, rows) {
  ...
  return savePlantCostSlotsForDay(projectId, dayId, nextSlots)
}
...
export function clearAllPlantCosts() {
  ...
  removeGroveItem(PLANT_COST_STORAGE_KEY)
  removeGroveItem(LEGACY_FUEL_COST_STORAGE_KEY)
}
