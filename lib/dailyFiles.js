...
export function parseProjectDayId(value) {
  const text = String(value ?? "").trim()
  if (/^\d{4}-\d{2}-\d{2}$/.test(text)) {
    return parseDayId(text)
  }

  const parsed = new Date(text)
  if (!text || Number.isNaN(parsed.getTime())) {
    return parseDayId(getTodayDayId())
  }

  return startOfDay(parsed)
}

export function dayIdFromTimestamp(value, timeZone = SITE_TIMEZONE) {
...
export const SITE_TIMEZONE = "Africa/Harare"

export function getTodayDayId(timeZone = SITE_TIMEZONE) {
  return new Intl.DateTimeFormat("en-CA", {
    timeZone,
    year: "numeric",
    month: "2-digit",
