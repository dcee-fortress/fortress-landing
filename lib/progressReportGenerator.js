...
export function formatDayLabelFromId(dayId) {
  if (!dayId) return ""
  return formatDayLabel(parseDayId(dayId))
}
...
export function createBlankDailyProgressReport(dayId, { inProgress = false, day } = {}) {
  const reportDay = day ?? parseDayId(dayId)

  return {
    id: dayId,
    label: `Daily Report - ${formatDayLabel(reportDay)}`,
    date: formatDayLabel(reportDay),
    createdAt: new Date().toISOString(),
    completedAt: inProgress ? null : formatShortDate(addDays(reportDay, 1)),
    progressSummary: "",
    progressUpdate: {
      content: "",
      attachments: [],
...
export function generateProgressReports(projectStart, throughDay) {
  ...
  const today = parseDayId(getTodayDayId())
...
export function generateDailyProgressReports(projectStart, throughDay) {
  const files = []
  let day = startOfDay(projectStart)
  const end = startOfDay(throughDay)
  const today = parseDayId(getTodayDayId())

  while (day <= end) {
    files.push(
      createBlankDailyProgressReport(`${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, "0")}-${String(day.getDate()).padStart(2, "0")}`, {
        day,
