...
async function publishLocalSharedKeys() {
  const skip = new Set([GROVE_STORAGE_KEYS.lastSession, GROVE_STORAGE_KEYS.bootstrapDone])
...
export async function deleteGroveProject(projectId) {
  ...
  if (removed) {
    try {
      await publishLocalSharedKeys()
    } catch {
      // Local delete still stands; the next sync will retry the live write.
    }
  }
