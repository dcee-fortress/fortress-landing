...
import { publishSharedKeys, replaceLiveSharedStorage } from "@/lib/sharedPersistence"
...
export async function deleteGroveProject(projectId) {
  ...
  if (removed) {
    try {
      await publishSharedKeys({ replace: true, includeTombstones: true })
    } catch {
      return { ok: false, message: "Deleted locally, but the live database could not be updated." }
    }
  }
...
export async function endGroveProject(projectId, endDate = null) {
  ...
  try {
    ...
    try {
      await publishSharedKeys()
    } catch {
      return {
        ok: false,
        message: `"${project.name}" ended on this device, but the live database could not be updated.`,
      }
    }
