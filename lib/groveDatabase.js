import { getGroveItem, removeGroveItem } from "@/lib/groveClientStore"
import { clearProjectsRegistry, deleteCustomProject, endCustomProject, SEEDED_PROJECT_ID } from "@/lib/projectRegistry"
import { clearAllBoqs, removeBoqsForProject } from "@/lib/boqData"
import { clearAllDescriptionMemory, removeDescriptionMemoryForProject } from "@/lib/boqDescriptionMemory"
import { clearAllFormulaMemory, removeFormulaMemoryForProject } from "@/lib/materialFormulaMemory"
import { resetDemoValuesPreference } from "@/lib/demoMode"
import { clearAllEquipmentHours, removeEquipmentHoursForProject } from "@/lib/equipmentHoursData"
import { GROVE_STORAGE_KEYS } from "@/lib/grovePersistence"
import { invalidateGroveCaches } from "@/lib/invalidateGroveCaches"
import { clearProgressPhotoStore } from "@/lib/progressReportPhotoStore"
import {
  clearAllMaterialSchedules,
  removeMaterialSchedulesForProject,
} from "@/lib/materialSchedule"
import { clearAllPlantCosts, removePlantCostForProject } from "@/lib/plantCostData"
import {
  clearAllPlantHours,
  clearPlantHoursForProject,
} from "@/lib/plantHoursData"
import {
  clearAllPlantOperatorRegisters,
  removePlantOperatorRegistersForProject,
} from "@/lib/plantOperatorRegisterData"
import {
  clearAllSiteStaffRegisters,
  removeSiteStaffRegistersForProject,
} from "@/lib/siteStaffRegisterData"
import {
  clearAllInductionRegisters,
  removeInductionRegistersForProject,
} from "@/lib/inductionRegisterData"
import { clearAllProjectData, removeProjectData } from "@/lib/projectData"
import { replaceLiveSharedStorage } from "@/lib/sharedPersistence"
import { emptySharedStorage } from "@/lib/sharedStorageMerge"
import { readResponseJson } from "@/lib/safeJson"

function clearProjectScopedData(projectId) {
  removeMaterialSchedulesForProject(projectId)
  removeProjectData(projectId)
  clearPlantHoursForProject(projectId)
  removeBoqsForProject(projectId)
  removeDescriptionMemoryForProject(projectId)
  removeFormulaMemoryForProject(projectId)
  removePlantCostForProject(projectId)
  removeEquipmentHoursForProject(projectId)
  removePlantOperatorRegistersForProject(projectId)
  removeSiteStaffRegistersForProject(projectId)
  removeInductionRegistersForProject(projectId)
}

export async function clearEntireGroveDatabase() {
  if (typeof window === "undefined") return { ok: false, message: "Not available on server." }

  clearAllProjectData()
  clearAllMaterialSchedules()
  clearAllBoqs()
  clearAllDescriptionMemory()
  clearAllFormulaMemory()
  clearAllPlantCosts()
  clearAllEquipmentHours()
  clearAllPlantHours()
  clearAllPlantOperatorRegisters()
  clearAllSiteStaffRegisters()
  clearAllInductionRegisters()
  resetDemoValuesPreference()
  clearProjectsRegistry()
  void clearProgressPhotoStore()
  removeGroveItem(GROVE_STORAGE_KEYS.lastSession)

  try {
    await replaceLiveSharedStorage(emptySharedStorage(), { forceEmpty: true })
  } catch {
    return { ok: false, message: "Cleared this browser, but the live database could not be cleared." }
  }

  invalidateGroveCaches()
  window.dispatchEvent(new Event("grove-shared-storage-change"))
  return { ok: true, message: "The live database has been cleared for every device." }
}

export async function deleteGroveProject(projectId, options = {}) {
  if (typeof window === "undefined") return { ok: false }

  if (projectId === SEEDED_PROJECT_ID) {
    clearProjectScopedData(projectId)
    return {
      ok: true,
      seeded: true,
      message: "Roads demo data has been reset. The built-in project remains available.",
    }
  }

  // Optimistic: update browser cache immediately, persist to Supabase in background.
  const removed = await deleteCustomProject(projectId, {
    waitForPersist: options.waitForPersist === true,
  })
  invalidateGroveCaches()
  window.dispatchEvent(new Event("grove-shared-storage-change"))

  if (removed) {
    void Promise.resolve().then(() => clearProjectScopedData(projectId))
  }

  return removed
    ? {
        ok: true,
        seeded: false,
        message: "Project permanently deleted from the live database.",
      }
    : { ok: false, message: "Could not delete this project." }
}

export function canDeleteProject(projectId) {
  return Boolean(projectId)
}

export async function endGroveProject(projectId, endDate = null) {
  if (typeof window === "undefined") return { ok: false, message: "Not available on server." }

  if (projectId === SEEDED_PROJECT_ID) {
    return { ok: false, message: "The built-in demo project cannot be ended from settings." }
  }

  try {
    // writeRegistry already publishes the project list to Supabase.
    const project = await endCustomProject(projectId, endDate)
    if (!project) {
      return { ok: false, message: "Could not end this project." }
    }

    invalidateGroveCaches()
    window.dispatchEvent(new Event("grove-shared-storage-change"))

    return {
      ok: true,
      message: `"${project.name}" ended on ${project.endDate}. All saved data is kept and no new dates will be added.`,
      project,
    }
  } catch (error) {
    return { ok: false, message: error.message ?? "Could not end project." }
  }
}

export async function exportGroveDatabaseBackup() {
  if (typeof window === "undefined") return

  const backup = {
    exportedAt: new Date().toISOString(),
    data: {},
  }

  let liveData = null
  try {
    const response = await fetch("/api/shared-storage", { cache: "no-store" })
    if (response.ok) liveData = await readResponseJson(response, null)
  } catch {
    liveData = null
  }

  for (const storageKey of Object.values(GROVE_STORAGE_KEYS)) {
    const value = liveData?.[storageKey] ?? getGroveItem(storageKey)
    if (value !== null) {
      backup.data[storageKey] = value
    }
  }

  const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" })
  const url = URL.createObjectURL(blob)
  const link = document.createElement("a")
  link.href = url
  link.download = `grove-backup-${new Date().toISOString().slice(0, 10)}.json`
  document.body.appendChild(link)
  link.click()
  link.remove()
  URL.revokeObjectURL(url)
}
