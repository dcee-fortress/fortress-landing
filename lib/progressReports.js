import { getProjectStartDate } from "@/lib/periodFiles"
import { getProjectEffectiveThroughDate } from "@/lib/projectRegistry"
import { shouldUseDemoValues } from "@/lib/demoMode"
import {
  buildInitialDemoTargetPlan,
  carryForwardProgressReportContent,
  fillDemoProgressReportContent,
  isProgressReportContentEmpty,
  isTargetPlanEmpty,
} from "@/lib/progressReportDemo"
import {
  createBlankProgressReport,
  createBlankDailyProgressReport,
  formatDayLabel,
  generateDailyProgressReports,
  generateProgressReportId,
  generateProgressReports,
  getWeekDateRange,
  formatWeekRange,
} from "@/lib/progressReportGenerator"
import { readProjectsRegistry, writeProjectsRegistry } from "@/lib/projectRegistry"
import { stripPhotosForStorage } from "@/lib/progressReportPhotos"
import {
  addDays,
  parseDayId,
  weekIdFromStart,
} from "@/lib/weeklyFiles"

export {
  createBlankProgressReport,
  createBlankDailyProgressReport,
  formatDayLabel,
  formatDayLabelFromId,
  generateDailyProgressReports,
  formatWeekRange,
  generateProgressReportId,
  generateProgressReports,
  getWeekDateRange,
} from "@/lib/progressReportGenerator"

export const PROGRESS_REPORTS_STORAGE_KEY = "grove-progress-reports"

const DAILY_PROGRESS_REPORTS_KEY = "dailyProgressReports"
const WEEKLY_PROGRESS_REPORTS_KEY = "weeklyProgressReports"

function startOfDay(date) {
  const next = new Date(date)
  next.setHours(0, 0, 0, 0)
  return next
}

function shouldSeedProgressReportDemo(projectId) {
  return shouldUseDemoValues(projectId)
}

function isReportInProgress(report) {
  const { endDate } = getWeekDateRange(report.id)
  return endDate >= startOfDay(new Date())
}

function getPreviousWeekReportId(weekId, projectStart) {
  const weekStart = parseDayId(weekId)
  const previousWeekStart = addDays(weekStart, -7)
  const projectStartDay = startOfDay(projectStart)

  if (previousWeekStart < projectStartDay) {
    return null
  }

  return weekIdFromStart(previousWeekStart)
}

function applyContentToNewReport(report, previousReport, projectId) {
  const inProgress = isReportInProgress(report)
  const useDemo = shouldSeedProgressReportDemo(projectId)

  if (inProgress) {
    if (previousReport) {
      return carryForwardProgressReportContent(report, previousReport)
    }

    if (useDemo) {
      const weekRange = report.weekRange ?? formatWeekRange(report.id)
      return {
        ...report,
        progressSummary: buildInitialDemoTargetPlan({ weekRange }),
        progressUpdate: {
          ...report.progressUpdate,
          content: "",
          updatedAt: new Date().toISOString(),
        },
      }
    }

    return report
  }

  if (useDemo && isTargetPlanEmpty(report)) {
    return fillDemoProgressReportContent(report)
  }

  return report
}
function mergeProgressReports(existingReports, generatedReports, projectId, projectStart) {
  const merged = new Map()

  for (const report of existingReports) {
    merged.set(report.id, report)
  }

  const sortedGenerated = [...generatedReports].sort((left, right) => left.id.localeCompare(right.id))

  for (const report of sortedGenerated) {
    if (merged.has(report.id)) {
      continue
    }

    const previousReportId = getPreviousWeekReportId(report.id, projectStart)
    const previousReport = previousReportId ? merged.get(previousReportId) : null
    merged.set(report.id, applyContentToNewReport(report, previousReport, projectId))
  }

  if (shouldSeedProgressReportDemo(projectId)) {
    for (const [reportId, report] of merged.entries()) {
      if (!isReportInProgress(report) && isTargetPlanEmpty(report)) {
        merged.set(reportId, fillDemoProgressReportContent(report))
      }
    }
  }

  for (const [reportId, report] of merged.entries()) {
    if (!isReportInProgress(report) || !isTargetPlanEmpty(report)) {
      continue
    }

    const previousReportId = getPreviousWeekReportId(reportId, projectStart)
    const previousReport = previousReportId ? merged.get(previousReportId) : null

    if (previousReport && !isTargetPlanEmpty(previousReport)) {
      merged.set(reportId, carryForwardProgressReportContent(report, previousReport))
      continue
    }

    if (shouldSeedProgressReportDemo(projectId)) {
      merged.set(reportId, fillDemoProgressReportContent(report))
    }
  }

  const today = startOfDay(new Date())
  const generatedIds = new Set(generatedReports.map((report) => report.id))

  return [...merged.values()]
    .filter((report) => generatedIds.has(report.id))
    .map((report) => {
      const { endDate } = getWeekDateRange(report.id)
      const inProgress = endDate >= today

      if (inProgress) {
        return {
          ...report,
          status: "in-progress",
          completedAt: null,
        }
      }

      if (report.status === "in-progress") {
        return {
          ...report,
          status: "completed",
          completedAt: report.completedAt ?? formatShortDate(addDays(endDate, 1)),
        }
      }

      return report
    })
    .sort((left, right) => right.id.localeCompare(left.id))
}

function mergeDailyProgressReports(existingReports, generatedReports, projectStart, throughDay, deletedDayIds = []) {
  const existingById = new Map((existingReports ?? []).map((report) => [report.id, report]))
  const deleted = new Set(deletedDayIds ?? [])
  const firstDayId = `${projectStart.getFullYear()}-${String(projectStart.getMonth() + 1).padStart(2, "0")}-${String(projectStart.getDate()).padStart(2, "0")}`
  const lastDayId = `${throughDay.getFullYear()}-${String(throughDay.getMonth() + 1).padStart(2, "0")}-${String(throughDay.getDate()).padStart(2, "0")}`

  return generatedReports
    .filter((report) => !deleted.has(report.id))
    .map((report) => ({
      ...report,
      ...(existingById.get(report.id) ?? {}),
      status: report.status,
      completedAt: report.status === "in-progress" ? null : report.completedAt,
    }))
    .filter((report) => report.id >= firstDayId && report.id <= lastDayId)
    .sort((left, right) => right.id.localeCompare(left.id))
}

/**
 * Get all progress reports for a project
 */
export function getProjectProgressReports(projectId) {
  if (typeof window === "undefined") {
    return []
  }

  try {
    const registry = readProjectsRegistry()
    const projectFiles = registry.files[projectId]

    if (!projectFiles) {
      return []
    }

    return projectFiles[WEEKLY_PROGRESS_REPORTS_KEY] ?? projectFiles.progressReports ?? []
  } catch {
    return []
  }
}

export function getProjectDailyProgressReports(projectId) {
  if (typeof window === "undefined") {
    return []
  }

  try {
    const registry = readProjectsRegistry()
    const deleted = new Set(registry.files[projectId]?.deletedDailyIds ?? [])
    return (registry.files[projectId]?.[DAILY_PROGRESS_REPORTS_KEY] ?? []).filter(
      (report) => report?.id && !deleted.has(report.id)
    )
  } catch {
    return []
  }
}

export function getProjectDailyProgressReport(projectId, reportId) {
  const report = getProjectDailyProgressReports(projectId).find((entry) => entry.id === reportId)
  return report ? { ...report, reportType: "daily" } : null
}

export function getProjectWeeklyProgressReports(projectId) {
  return getProjectProgressReports(projectId)
}

/**
 * Get a specific progress report
 */
export function getProjectProgressReport(projectId, reportId) {
  const reports = getProjectProgressReports(projectId)
  const report = reports.find((entry) => entry.id === reportId)
  return report ? { ...report, reportType: "weekly" } : null
}

/**
 * Save or update a progress report
 */
export function saveProgressReport(projectId, report) {
  if (typeof window === "undefined") {
    return false
  }

  try {
    const registry = readProjectsRegistry()
    const projectFiles = registry.files[projectId]

    if (!projectFiles) {
      return false
    }

    const reportType = report.reportType === "daily" ? "daily" : "weekly"
    const reportsKey = reportType === "daily" ? DAILY_PROGRESS_REPORTS_KEY : WEEKLY_PROGRESS_REPORTS_KEY
    const reports = projectFiles[reportsKey] || projectFiles.progressReports || []
    const persistable = stripPhotosForStorage({ ...report, updatedAt: new Date().toISOString() })
    const existingIndex = reports.findIndex((entry) => entry.id === report.id)

    if (existingIndex >= 0) {
      reports[existingIndex] = {
        ...reports[existingIndex],
        ...persistable,
      }
    } else {
      reports.push({ ...persistable, createdAt: new Date().toISOString() })
    }

    projectFiles[reportsKey] = reports
    registry.files[projectId] = projectFiles
    return Promise.resolve(writeProjectsRegistry(registry)).then(() => true)
  } catch (error) {
    if (error instanceof Error) {
      throw error
    }
    return false
  }
}

/**
 * Ensure daily and weekly Word progress reports exist through today,
 * matching the daily valuation files that feed those documents.
 */
export function ensureProgressReportsExist(projectId) {
  if (typeof window === "undefined") {
    return false
  }

  try {
    const registry = readProjectsRegistry()
    let createdFilesBucket = false

    if (!registry.files[projectId]) {
      registry.files[projectId] = {
        daily: [],
        weekly: [],
        monthly: [],
        progressReports: [],
        weeklyProgressReports: [],
        dailyProgressReports: [],
      }
      createdFilesBucket = true
    }

    const projectStart = getProjectStartDate(projectId)
    const throughDayId = getProjectEffectiveThroughDate(projectId)
    const through = parseDayId(throughDayId)
    const generated = generateProgressReports(projectStart, through)
    const generatedDaily = generateDailyProgressReports(projectStart, through)
    const existing = registry.files[projectId].weeklyProgressReports || registry.files[projectId].progressReports || []
    const existingDaily = registry.files[projectId].dailyProgressReports || []
    const deletedDailyIds = registry.files[projectId].deletedDailyIds || []
    const nextWeekly = mergeProgressReports(existing, generated, projectId, projectStart)
    const nextDaily = mergeDailyProgressReports(
      existingDaily,
      generatedDaily,
      startOfDay(projectStart),
      through,
      deletedDailyIds
    )

    const weeklyIds = (reports) =>
      (reports ?? []).map((report) => report.id).sort().join("|")
    const changed =
      createdFilesBucket ||
      weeklyIds(nextWeekly) !== weeklyIds(existing) ||
      weeklyIds(nextDaily) !== weeklyIds(existingDaily)

    if (!changed) {
      return false
    }

    registry.files[projectId].weeklyProgressReports = nextWeekly
    registry.files[projectId].progressReports = nextWeekly
    registry.files[projectId].dailyProgressReports = nextDaily
    writeProjectsRegistry(registry)
    return true
  } catch {
    return false
  }
}

/**
 * Delete a progress report
 */
export function deleteProgressReport(projectId, reportId) {
  if (typeof window === "undefined") {
    return false
  }

  try {
    const registry = readProjectsRegistry()
    const projectFiles = registry.files[projectId]

    if (!projectFiles) {
      return false
    }

    const reports = projectFiles.progressReports || []
    projectFiles.progressReports = reports.filter((report) => report.id !== reportId)
    registry.files[projectId] = projectFiles
    writeProjectsRegistry(registry)

    return true
  } catch {
    return false
  }
}

/**
 * Add an attachment to a progress report
 */
export function addAttachment(projectId, reportId, attachment) {
  const report = getProjectProgressReport(projectId, reportId)
  if (!report) return false

  const updated = {
    ...report,
    attachments: [...(report.attachments || []), attachment],
  }

  return saveProgressReport(projectId, updated)
}

/**
 * Remove an attachment from a progress report
 */
export function removeAttachment(projectId, reportId, attachmentId) {
  const report = getProjectProgressReport(projectId, reportId)
  if (!report) return false

  const updated = {
    ...report,
    attachments: report.attachments.filter((attachment) => attachment.id !== attachmentId),
  }

  return saveProgressReport(projectId, updated)
}
