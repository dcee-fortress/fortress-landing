...
import { seedLocalDevValuationDemo } from "@/lib/demoRateAnalysisSeed"
...
function purgeDemoFromLocalStorage() {
  if (typeof window === "undefined") return false
...
function bootstrapProjectIfNeeded(projectId) {
  ...
  if (window.sessionStorage.getItem(projectKey) === "1") {
    return seedLocalDevValuationDemo(projectId)
  }

  bootstrapProject(projectId)
  const demoSeeded = seedLocalDevValuationDemo(projectId)
...
const REGISTRY_MIGRATION_KEY = "grove-registry-migration-v3-done"
...
export function initializeGrovePersistence(options = {}) {
  ...
  const demoPurged = purgeDemoFromLocalStorage()
  ...
  if (!force && bootstrapDone) {
    const calendarChanged = bootstrapCalendarFiles()
    if (projectId) {
      const changed =
    ...
    const demoSeeded = seedLocalDevValuationDemo()
    const changed =
      calendarChanged || chadcomSlotsAdded || registryMigrated || materialScheduleMigrated || demoSeeded || demoPurged
