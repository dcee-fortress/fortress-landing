...
import { purgeTargetCostMaterialSchedules, MATERIAL_SCHEDULE_STORAGE_KEY } from "@/lib/materialSchedule"
import { PROJECT_DATA_STORAGE_KEY } from "@/lib/projectData"
import { PROJECTS_REGISTRY_KEY, getCustomProjects, migrateRegistryProjects } from "@/lib/projectRegistry"
...
function getProjectBootstrapKey(projectId) {
  return `grove-bootstrap-${projectId}`
}
...
export function initializeGrovePersistence(options = {}) {
  ...
  if (!force && bootstrapDone) {
    if (projectId) {
      const changed = bootstrapProjectIfNeeded(projectId) || chadcomSlotsAdded || registryMigrated
      return { ok: true, changed, skipped: !changed }
    }

    return { ok: true, changed: chadcomSlotsAdded || registryMigrated, skipped: !(chadcomSlotsAdded || registryMigrated) }
  }
