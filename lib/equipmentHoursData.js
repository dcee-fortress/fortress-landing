...
export function enrichEquipmentWithHours(projectId, dayId, equipment) {
  ...
  return equipment.map((item) => {
    const stored = dayData.entries[item.id] ?? { startHours: "", finishHours: "" }
    const startHours = normalizeHourField(stored.startHours)
    const finishHours = normalizeHourField(stored.finishHours)
...
function dayHasEquipmentHours(projectId, dayId) {
  ...
  return Object.values(dayData.entries).some((entry) => {
    const hours = calculateOperatingHours(entry.startHours, entry.finishHours)
...
export function getEquipmentHoursProjectToDateSummary(projectId) {
  ...
  for (const dayId of dayIds) {
    ...
    for (const entry of Object.values(dayData.entries)) {
      totalHours += calculateOperatingHours(entry.startHours, entry.finishHours) ?? 0
    }
  }
...
export function getEquipmentHoursDetailDescription(period) {
  if (period === "daily") {
    return "Equipment is listed from register ticks marked present. Plant names link from material schedule entries for the same day (editable here). Enter start and finish hours — hours operating calculates automatically."
  }
