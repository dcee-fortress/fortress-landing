...
function readStore() {
  ...
  try {
    const raw = getGroveItem(EQUIPMENT_HOURS_STORAGE_KEY)
...
function writeStore(store) {
  ...
  return import("@/lib/saveToPostgres").then(({ writeGroveJson }) =>
    writeGroveJson(EQUIPMENT_HOURS_STORAGE_KEY, store, { replace: true })
  )
}
...
export function enrichEquipmentWithHours(projectId, dayId, equipment) {
  ...
  return equipment.map((item) => {
    const stored = findStoredEquipmentHours(dayData, item, equipmentCount)
...
function entryHasHourValues(entry) {
  const normalized = normalizeEquipmentHoursEntry(entry)
  return Boolean(normalized.startHours || normalized.finishHours || normalized.hoursOperating)
}

function findStoredEquipmentHours(dayData, item, equipmentCount) {
  const entries = dayData.entries ?? {}
  const direct = entries[item.id]
  if (direct && entryHasHourValues(direct)) {
    return normalizeEquipmentHoursEntry(direct)
  }

  const plantNumber = String(item.plantNumber || "").trim().toLowerCase()
  const plant = String(item.plant || "").trim().toLowerCase()
  const matches = Object.entries(entries).filter(([id, entry]) => {
    const normalized = normalizeEquipmentHoursEntry(entry)
    if (plantNumber && id.trim().toLowerCase() === plantNumber) return true
    if (plant && normalized.plant && normalized.plant.toLowerCase() === plant) return true
    return false
  })

  const withHours = matches.filter(([, entry]) => entryHasHourValues(entry))
  if (withHours.length === 1) {
    return normalizeEquipmentHoursEntry(withHours[0][1])
  }
  if (matches.length === 1) {
    return normalizeEquipmentHoursEntry(matches[0][1])
  }

  const hourEntries = Object.values(entries).filter(entryHasHourValues)
  if (hourEntries.length === 1 && equipmentCount === 1) {
    return normalizeEquipmentHoursEntry(hourEntries[0])
  }
...
export function getEquipmentHoursDetailDescription(period) {
  if (period === "daily") {
    return "Equipment is listed from operators marked present in the register on this date. Supplier, plant name, and plant number copy from that register row. Enter start and finish hours, or type hours operating directly."
  }
...
export function clearAllEquipmentHours() {
  if (typeof window === "undefined") return
  removeGroveItem(EQUIPMENT_HOURS_STORAGE_KEY)
}
