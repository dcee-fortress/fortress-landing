...
const MANUAL_MODE_FLAG = "__ppeIssuedManualModeV1__"
...
function buildPpeIssuedFile(dayId) {
  const today = getTodayDayId()
  return {
    id: dayId,
    dayId,
    label: formatPpeIssuedDateLabel(dayId) || dayId,
    date: dayId,
    isToday: dayId === today,
  }
}
 ...
 */
export async function migratePpeIssuedToManualOnly(projectId) {
  ...
  const withData = getPpeIssuedDayIds(projectId)
  ...
  await ppeIssuedStore.write(next)
  return true
}

/** Files appear only after the user presses +. */
export function getPpeIssuedDailyFiles(projectId) {
  if (typeof window === "undefined" || !projectId) return []
  const store = ppeIssuedStore.read()
  const deleted = new Set(readDeletedDayIds(store, projectId))
  return readManualDayIds(store, projectId)
    .filter((dayId) => !deleted.has(dayId))
    .map(buildPpeIssuedFile)
}

export function getPpeIssuedDailyFile(projectId, dayId) {
  if (!projectId || !dayId || isPpeIssuedDayDeleted(projectId, dayId)) return null
  if (typeof window === "undefined") return null
  return getPpeIssuedDailyFiles(projectId).find((file) => file.id === dayId) ?? null
}

/**
 * Creates today's PPE issued daily file. One file per calendar day.
 */
export async function createPpeIssuedDailyFileForToday(projectId) {
  ...
  if (manual.has(dayId) && !deleted.has(dayId)) {
    return {
      ...
      dayId,
      alreadyExists: true,
      file: buildPpeIssuedFile(dayId),
      message: "Today's PPE issued file already exists.",
    }
  }
