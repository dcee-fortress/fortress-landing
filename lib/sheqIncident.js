...
const MANUAL_MODE_FLAG = "__incidentManualModeV2__"
/**
...
/**
 * One-time cleanup: drop empty stubs so the list starts blank until the user
 * presses +. Keeps only reports that already have real entered content.
 */
export async function migrateSheqIncidentToManualOnly(projectId) {
  ...
  for (const fileId of listed) {
    ...
    if (reportHasEnteredContent(next[key])) {
  ...
  for (const key of Object.keys(next)) {
    ...
    if (reportHasEnteredContent(next[key])) {
      if (!keepManual.includes(fileId)) keepManual.push(fileId)
      continue
    }
    delete next[key]
  }
  ...
  // Keep existing tombstones — clearing them lets sync resurrect deleted files.
...
export async function sealSheqIncidentDeletes(projectId) {
  ...
  for (const fileId of [...manual]) {
    if (!isFileAlive(next, projectId, fileId)) {
...
export function getSheqIncidentDailyFiles(projectId) {
  ...
  const flags = blob[MANUAL_MODE_FLAG]
  const inManualMode = Boolean(flags && typeof flags === "object" && flags[projectId])
  const legacy = inManualMode ? [] : readIdList(blob, LEGACY_CREATED_KEY, projectId)
  const ids = new Set([...manual, ...legacy])
  return [...ids]
    .filter((id) => isFileAlive(blob, projectId, id))
