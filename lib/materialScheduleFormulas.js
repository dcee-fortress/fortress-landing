...
export function draftFormulaInput(draft) {
  const text = String(draft ?? "")
  return {
    value: text,
    formula: isFormula(text) ? text.trim() : "",
  }
}
...
export function resolveNumericField(rawRows, resolvedGrid, rowIndex, columnKey) {
  ...
  return isFormula(raw) ? evaluateFormula(raw) : parsePlainNumber(raw)
}
...
export function getMaterialFormulaHelpText() {
  return "Enter a number or a formula with figures only (e.g. =3*4, =100+50). Formulas starting with = are remembered; type = again on a similar activity to pick a saved formula (Tab to use, X to close). The answer appears in the cell; click the cell to see the formula in the bar above. Auto-calculated fields fill in when left blank."
}
