...
function idsKey(items = []) {
  // Preserve list order so a mis-sorted calendar (today stuck at the bottom)
  // is detected as a change and rewritten newest-first.
...
function fillProjectCalendarFiles(projectFiles, project, todayId) {
  ...
  const deletedDailyIds = new Set(
    (bucket.deletedDailyIds ?? []).filter((id) => typeof id === "string" && id)
  )
  ...
  while (cursor <= through) {
    ...
    if (!existingDailyIds.has(id) && !deletedDailyIds.has(id)) {
  ...
  return {
    ...
    files: {
      ...bucket,
      deletedDailyIds: [...deletedDailyIds],
      daily: nextDaily,
...
export function applyCalendarFilesToRegistry(registry, todayId = getTodayDayId()) {
  ...
  for (const project of projects) {
    ...
    if ((registry.deletedIds ?? []).includes(project.id)) continue
...
export function mergeProjectRegistries(existingRegistry, incomingRegistry, todayId = getTodayDayId()) {
  ...
  for (const projectId of projectIds) {
    ...
    files[projectId] = {
      ...
      deletedDailyIds: [
        ...new Set([
          ...(previous.deletedDailyIds ?? []),
          ...(next.deletedDailyIds ?? []),
        ]),
      ],
      daily: mergeById(previous.daily, next.daily).filter(
        (file) =>
          !(previous.deletedDailyIds ?? []).includes(file.id) &&
          !(next.deletedDailyIds ?? []).includes(file.id)
      ),
