...
async function resolvePhotoDataUrl(photo) {
  if (typeof photo?.data === "string" && photo.data.startsWith("data:")) return photo.data
  const src =
    (typeof photo?.url === "string" && photo.url) ||
    (photo?.id ? `/api/progress-photos?id=${encodeURIComponent(String(photo.id))}` : "")
  if (!src) return ""
...
async function appendPhotoPages(pdf, photos) {
  ...
  for (const photo of photos || []) {
    ...
    try {
      pdf.addImage(dataUrl, imageFormatFromDataUrl(dataUrl), x, y, drawWidth, drawHeight, undefined, "FAST")
    } catch {
